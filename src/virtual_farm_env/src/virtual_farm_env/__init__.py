...
def get_driving_pattern_file() -> str:
    driving_pattern_path = (
        Path(_cache_dir) / "virtual_farm_env" / "driving_pattern.txt"
    )
    ...
    if not driving_pattern_path.is_file():
        raise NoWorldGeneratedException(
            ...
            " 'rosrun virtual_farm_env generate_world.py fre22_task_navigation_mini'"
            " (for example)."
        )
...
def get_markers_file() -> str:
    markers_path = Path(_cache_dir) / "virtual_farm_env" / "markers.csv"

    if not markers_path.is_file():
        raise NoWorldGeneratedException(
            "No markers file is generated! First generate marker file by"
            " 'rosrun virtual_farm_env generate_world.py fre21_task_3'"
            " (for example)."
        )
